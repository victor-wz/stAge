"""Fig S9 -- Module-specific clocks (mouse tissue panel).

# INFERRED (unconfirmed): see fig6e_module_clocks.py's module docstring for the full
explanation of this inference and its caveats -- this file is a copy of that same
mouse-tissue placeholder (output filename changed) pending confirmation of the real
per-figure dataset assignment with the author.

CLI:
    python figS9_module_clocks.py --rawdata-dir DIR --ncbi-reference-path PATH \\
...
def main():
    ...
    summary.to_csv(os.path.join(args.save_dir, 'figS9_module_clocks_summary.csv'), index=False)
    print(summary)
